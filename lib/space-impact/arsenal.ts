...
export function isWeapon(value: unknown): value is Weapon {
  return WEAPON_ORDER.includes(value as Weapon);
}
/** Old saves retain their equipped gun; untrusted levels stay in 0..3. */
export function restoreArsenal(
  raw: unknown,
  equipped: Weapon = "pulse",
  level = 1,
): Arsenal {
  ...
  if (raw && typeof raw === "object")
    for (const weapon of WEAPON_ORDER) {
      ...
      if (typeof n === "number" && Number.isFinite(n))
        arsenal[weapon] = Math.min(3, Math.max(0, Math.floor(n)));
    }
  arsenal.pulse = Math.max(1, arsenal.pulse);
  arsenal[equipped] = Math.min(
    3,
