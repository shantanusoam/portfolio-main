...
/**
 * Keeps serverless deployments operational even when the platform does not run
 * a migration command during build. The SQL is idempotent; the checked-in
 * migration remains the source of truth for managed deployments.
 */
export async function ensureControlPlaneSchema(): Promise<boolean> {
  const pool = getDbPool();
  if (!pool) return false;

  if (!bootstrapPromise) {
    bootstrapPromise = pool.query(CONTROL_PLANE_SCHEMA).then(() => undefined);
  }
