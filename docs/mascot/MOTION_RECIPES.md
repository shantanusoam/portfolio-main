...
| `wander`        | 0.5s            | 40s             | The long ceiling is a safety valve, not a target — normal exits happen via curiosity/inspection hints, obstacle avoidance, or pointer activity. `rest-curl` is now a moving local loop rather than a freeze. |
| `sprint`        | 0.5s            | 3s              | Short burst per spec ("Autonomous sprints must be brief").                                                                                                                                                   |
| `rest`          | 1.1s            | 5.5s            | A brief authored settle after pointer release or recovery; autonomous wandering resumes before the fish can appear frozen.                                                                                   |
