...
import { Gamepad2, Pause, Play, RotateCcw, Volume2, X } from "lucide-react";
...
function drawAtlasSprite<K extends string>(
...
export default function SecretArcade() {
  ...
  const [hud, setHud] = useState<HudState>({
    ...
    lives: 3,
    multiplier: 1,
    powerLine: "Collect crates for chaos",
    score: 0,
    status: "ready",
    wave: 0,
    weapon: "pulse",
  ...
  const playSfx = useCallback((kind: SfxKind) => {
    ...
    oscillator.type =
      kind === "shoot" ? "square" : kind === "pickup" ? "triangle" : "sawtooth";
    oscillator.frequency.setValueAtTime(frequencies[kind], now);
    oscillator.frequency.exponentialRampToValueAtTime(
      kind === "pickup"
        ? frequencies[kind] * 1.8
        : Math.max(40, frequencies[kind] * 0.42),
      now + 0.08,
    );
    gain.gain.setValueAtTime(kind === "shoot" ? 0.015 : 0.04, now);
  ...
  const syncHud = useCallback((model: GameModel) => {
    ...
    setHud({
      ...
      powerLine: powerLine(model.player),
  ...
  const sizeCanvas = useCallback(() => {
    ...
    const width = Math.max(
      320,
      Math.floor(rect?.width || window.innerWidth || 900),
    );
    const height = Math.max(
      420,
      Math.floor(rect?.height || window.innerHeight || 720),
    );
  ...
  const openArcade = useCallback((source: string) => {
    setOpen(true);
    setUnlockFlash(
      source === "konami" ? "Konami coop unlocked" : "Secret coop unlocked",
    );
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (open) {
        ...
        if (
          ...
          (hud.status === "ready" || hud.status === "ended")
        ) {
          startGame();
          return;
        }
      ...
      konamiRef.current = [...konamiRef.current, event.code].slice(
        -KONAMI.length,
      );
      const hasKonami = KONAMI.every(
        (code, index) => konamiRef.current[index] === code,
      );
      if (hasKonami) {
        openArcade("konami");
        konamiRef.current = [];
      ...
      if (event.key.length === 1) {
        wordBufferRef.current = `${
          wordBufferRef.current
        }${event.key.toLowerCase()}`.slice(-16);
  ...
  }, [closeArcade, hud.status, open, openArcade, startGame, togglePause]);
  ...
  useEffect(() => {
    ...
    function tick(time: number) {
      ...
      if (model.sfxQueue.length) {
        const sounds = model.sfxQueue.splice(0, 3);
  ...
  }, [hud.status, open, playSfx, syncHud]);
  ...
  const statusText =
    hud.status === "running"
      ? `Wave ${hud.wave || 1} active`
      : hud.status === "paused"
        ? "Paused"
        : hud.status === "ended"
          ? "Run archived"
          : "Private coop waiting";
  const menuTitle =
    hud.status === "ended"
      ? "The flock got through"
      : hud.status === "paused"
        ? "Paused"
        : "Cluckstorm";
  const menuCopy =
    hud.status === "ended"
      ? "Restart with your weapon memory fresh. The next wave scales harder."
      : hud.status === "paused"
        ? "Catch your breath. The eggs can wait."
        : "A hidden vertical shooter with goofy alien space-birds, weapon crates, combo greed, and boss waves.";
        ...
        <div className={styles.overlay} role="dialog" aria-modal="true">
          ...
          >
            ...
            <header className={styles.hud}>
              <div className={styles.identity}>
                <span className={styles.eyebrow}>
                  Secret arcade / original shooter
                </span>
            ...
            <div className={styles.sideHud}>
              <div className={styles.pill}>Lives {hud.lives}</div>
              <div className={styles.pill}>
                {hud.weapon.toUpperCase()} LV {hud.weaponLevel}
              </div>
              <div className={styles.pill}>{hud.powerLine}</div>
            </div>
            ...
            {hud.bossMaxHp > 0 && (
              <div className={styles.bossMeter}>
                ...
                <div>
                  <i
                    style={{
                      transform: `scaleX(${hud.bossHp / hud.bossMaxHp})`,
                    }}
                  />
                </div>
              </div>
            ...
            <footer className={styles.bottomBar}>
              ...
              <span>Move: WASD / arrows / drag · fire auto · P pause</span>
              <span className={styles.assetChip}>
                <Volume2 size={13} aria-hidden="true" />
                Runtime assets {assetCount}/6
              </span>
            </footer>
                ...
                <div className={styles.menuCard}>
                  ...
                  <div className={styles.actionRow}>
                    <button
                      ...
                      type="button"
                      onClick={
                        hud.status === "paused" ? togglePause : startGame
                      }
                    >
